...
void
BrinkmanAdvDiffSemiImplicitHierarchyIntegrator::getFromInput(Pointer<Database> db, bool is_from_restart)
{
#if !defined(NDEBUG)
    TBOX_ASSERT(db);
#endif
    if (!is_from_restart)
    {
        AdvDiffSemiImplicitHierarchyIntegrator::getFromInput(db, is_from_restart);
