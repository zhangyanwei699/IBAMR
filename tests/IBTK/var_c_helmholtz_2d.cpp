...
main(int argc, char* argv[])
{
    ...
    { // cleanup dynamically allocated objects prior to shutdown
        ...
        poisson_spec.setCPatchDataId(C_coef_idx);
        ...
        if (IBTK_MPI::getRank() == 0)
        {
            ...
            out << "Error in u:L1-norm = " << std::setprecision(10) << e_vec.L1Norm() << std::endl;
            out << "Error in u:L2-norm = " << std::setprecision(10) << e_vec.L2Norm() << std::endl;
            out << "Error in u:max-norm = " << std::setprecision(10) << e_vec.maxNorm() << std::endl;
        }
